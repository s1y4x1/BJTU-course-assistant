(function initAcademicOptionsModule(global) {
  ...
  function applyAcademicDataCache(cache, studentId = '') {
    ...
    for (const result of (scheduleCache?.results || [])) {
      if (result?.type !== 'selection') continue;
      const hasSemesterResult = (scheduleCache?.results || []).some((item) => (
        item?.type === 'semester' && item?.xnxq === result.xnxq
      ));
      if (!hasSemesterResult) loadedScheduleTerms.delete(String(result.xnxq || ''));
    }
...
`;
  ...
  async function deleteExternalScript(script) {
    ...
    try {
      const knownSizeBytes = state.remoteSizeBytes || state.localSizeBytes;
      const manager = await getUpdaterManager();
      const root = await manager.requestDirectory();
      await manager.removeManagedFile(root, script.path);
      Object.assign(state, { installed: false, localSizeBytes: 0, remoteSizeBytes: knownSizeBytes, prefetchedSource: null, runtimeReady: false, reloadRequired: false, busy: false });
      renderExternalScriptState(script);
  ...
  async function loadAll({ preserveRendered = false, refreshCached = false } = {}) {
    ...
    try {
      ...
      if (!sharedPriority || !allTerms.length) throw new Error('没有可查询的学期');
      if (selected === '__all__') {
        renderCachedSharedData();
