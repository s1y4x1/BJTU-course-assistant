...
(function initQwenChatApp(global) {
  ...
  function buildQwenFileSizeEmphasisStyle(bytes) {
  ...
  function applyQwenFileSizeEmphasis(element, bytes) {
    if (!(element instanceof HTMLElement)) return;
    const style = typeof global.buildFileSizeEmphasisStyle === 'function'
      ? global.buildFileSizeEmphasisStyle(bytes)
      : buildQwenFileSizeEmphasisStyle(bytes);
    if (typeof global.applyEmphasisStyle === 'function') {
      global.applyEmphasisStyle(element, style);
      return;
    }
    for (const property of ['font-size', 'font-weight', 'color', 'text-shadow']) {
      const value = String(style || '').match(new RegExp(`${property}:([^;]+)`))?.[1]?.trim();
  ...
  function createOperationResultSize(text) {
    ...
    applyQwenFileSizeEmphasis(size, bytes);
    return size;
  }

  global.addEventListener('bjtu-theme-change', () => {
    document.querySelectorAll('.qwen-chat-op-size[data-file-size-bytes]').forEach((size) => {
      applyQwenFileSizeEmphasis(size, Number(size.dataset.fileSizeBytes || 0));
    });
  });
