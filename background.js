    ...
    chrome.scripting.executeScript({
      ...
      func: async (context) => {
        ...
        async function autoRecognizeCaptchaCode() {
          ...
          // 3) try TextDetector OCR on captcha image (if available)
          ...
          // 4) fallback to Tesseract OCR (same family as app.html)
          try {
        ...
        // Detect portal credential failure for extension-triggered login:
        // If default password failed on requested account, switch to aux account 8888 first,
        // then let app page switch back to target account with captcha flow.
        ...
        // Show injected modal whenever username or captcha is missing.
        // - fromExtension=true + username prefilled + no passcode: modal shows captcha OCR/retry flow
        // - normal portal open: modal asks for both account and captcha
        if (!username || !passcode) {
              ...
              <div style="margin-top:10px;display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                <div style="border:1px solid #e5e7eb;border-radius:10px;padding:10px;background:#f8fafc;">
                  ...
                  <div style="display:flex;gap:8px;justify-content:flex-end;">
                    <button id="__bjtu_refresh__" style="padding:6px 12px;">刷新验证码</button>
                    <button id="__bjtu_go" style="padding:6px 12px;background:#2563eb;color:#fff;border:0;border-radius:6px;">登录</button>
                  </div>
                </div>
          ...
          const got = await new Promise((resolve) => {
            ...
            btnGo.addEventListener('click', async () => {
              ...
              // Auto start when triggered from extension OR retry state is active.
              // User can cancel submit panel and return to input modal once without auto-start.
              ...
              if (!suppressAutoStartOnce && canAutoRetry && (fromExtension || !!flowState.forceRetry) && String(userInput.value || '').trim()) {
